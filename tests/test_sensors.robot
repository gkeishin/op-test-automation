...
Verify Sensor hwmon driver
    ...
    Run Keyword If  ${rc} == 0    Log To Console    Driver build into kernel

    #  OS release
    ${release}=   Get os info    cat /etc/os-release
    Log To Console     \n ${release}

    # loads ibmpowernv driver only on powernv platform
    Run Keyword if   '${release}' != 'PowerKVM'   lpar load ibm power nv

    # Checking for sensors command and lm_sensors package
    check sensor on lpar

    Run Keyword if   '${release}' != 'Ubuntu'   lpar check pkg for utility   dpkg -S
    ...   ELSE   Run Keyword    lpar check pkg for utility   rpm -qf

    # Restart the lm_sensor service
    @{lm_list} =   Create List    stop   start  status
    :FOR  ${cmd}   IN    @{lm_list}
    \    lpar start lm sensor svc   ${cmd}
...
lpar load ibm power nv
    ...
    Log To Console      ibmpowernv module is loaded
